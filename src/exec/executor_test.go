...
func TestBoundedQueueSizeInvariant(t *testing.T) {
	if got := boundedQueueSize(); got != 64 {
		t.Fatalf("boundedQueueSize() = %d, want 64", got)
	}
	if queueBufferSize != 64 {
		t.Fatalf("queueBufferSize = %d, want 64", queueBufferSize)
	}
}
